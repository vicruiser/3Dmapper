...
def pipeline(f): 
    ...
    process1 = subprocess.call(["Rscript %s %s %s" % (rscript1, f, chainseqs_outdir, args.pdb2pdb) ], shell=True)
    
    if args.pdb2pdb is True: 
    
        process1b = subprocess.call(["cat %s*.fasta > %s/%s" % (chainseqs_outdir,chainseqs_outdir, "pdb_chains.fasta") ], shell=True)
    
        process1c = subprocess.call(["makeblastdb -in %s -out %s -dbtype %s" % (chainseqs_outdir + "/pdb_chains.fasta",
                                                                            chainseqs_outdir+ "pdb_chains",
                                                                            "protein") ], shell=True)
        blastdb = chainseqs_outdir+ "pdb_chains"
    ...
    for file in glob.glob(os.path.join(chainseqs_outdir  ,(pdbid + "*_chain*.fasta"))): 
        ...
        -out %s.blast" % (file, blastdb, os.path.join(blast_outdir , os.path.basename(file)))] , shell =True, stdout=subprocess.PIPE,
