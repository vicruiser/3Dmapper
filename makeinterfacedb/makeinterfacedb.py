...
def pipeline(f): 
    ...
    for file in glob.glob(os.path.join(chainseqs_outdir  ,"*_chain*.fasta")): 
        ...
        if err2 is None and out2 == b'':
            ...
            if "BLAST hits didn't pass the filtering step." not in err3.decode('utf8'):
                ...
                process4 = subprocess.Popen(["Rscript %s %s %s %s %s %s %s %s" % (rscript3, root, f, interfaces_outdir, args.dist, args.type, args.int, args.biolip)] , shell =True)
