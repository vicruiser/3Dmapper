...
new.Mapped.Interfaces <-
  ...
  #  pi_ligand$chain.1 <- pi_ligand$resid.1
   ...
   # pi_all_unique <-
   #   merge(pi_all_unique, unique(min_max_mean_dist), by = c("resno", "resno.1"))
    ...
    # 2) Get mapped position
     ...
     new_mpi_template_split <- split(new_mpi, new_mpi[,c('chain','prot.id', 'qstart','sstart')], drop =TRUE)
    ...
    new_mpi_template_split <- lapply(new_mpi_template_split, 
           function(x){
             ...
             df<- df %>% group_by(qgaps) %>% mutate(qpos = cumsum(qgaps) + unique(x$qstart) -1)
             df<- df %>% group_by(sgaps) %>% mutate(spos = cumsum(sgaps) + unique(x$sstart) -1)
             ...
             #df$q_ali_pos = unique(x$qstart):(nrow(df)+ unique(x$qstart)-1)
             #df$s_ali_pos = unique(x$sstart):(nrow(df)+ unique(x$sstart)-1)
             #df$ali_pos = 1:nrow(df)
             #df$s_ali_pos = 1:nrow(df)
            ...
            # setnames(x,"q_ali_pos", "mapped.real.pos" )
             return(x)
           })
    new_mpi <- rbindlist(new_mpi_template_split)
    #new_mpi<- subset(new_mpi, q_ali_pos != 0)
    # # 3) Seq position
    # new_mpi$seq.pos <-
    ...
    # 
    new_mpi=subset(new_mpi, resid_qseq !='-' & resid_sseq != '-' )
    return(new_mpi)
  }
