...
def PDBmapper(protid,  geneid, transcriptid, psdb, vardb, out_dir, pident, isoform, APPRIS, UniprotID, consequence, loc, varid=None,csv= False, hdf = False):
    '''
    ...
    try: 
        ...
        logger.info('Variants file from gene id ' + transcriptid + ' parsed.')
    ...
    if psdf is False and annovars is False:
        ...
        if loc: 
            ...
            try: 
                noncoding_variants_index = annovars.Protein_position.str.contains(
                '\.|\-', regex=True, na=False)
            ...
            if noncoding_variants is not False:
                ...
                writefile(protid, out_dir, pident, isoform, consequence, noncoding_variants, 'NoncodingVariants', csv, hdf)
            ...
            if unmapped_variants.empty is False:
                ...
                writefile(protid, out_dir, pident, isoform, consequence, unmapped_variants, 'UnmappedVariants', csv, hdf)
        ...
        if loc:
            ...
            if left_variants.empty is False: 
                ...
                if structure_variants.empty is False:
                    ...
                    writefile(protid, out_dir, pident, isoform, consequence, structure_variants, 'StructureVariants', csv, hdf)
                    ...
                    try: 
                        ...
                        if unmapped_variants.empty is False:
                            ...
                            unmapped_variants.drop_duplicates(inplace=True)
                            unmapped_variants['Mapping_position'] = 'Unmapped'
                            unmapped_variants= unmapped_variants.drop(columns=['Uniprot_accession'])
                            writefile(protid, out_dir, pident, isoform, consequence, unmapped_variants, 'UnmappedVariants', csv, hdf)
                    except: 
                        pass
                ...
                del (structure_variants,left_variants,noncoding_variants, noncoding_variants_index)#,
            #      unmapped_variants,   )
        ...
        # save the merged dataframe as well
            ...
            with open(os.path.join(out_dir, ('setID_pident' + str(pident) + '_isoform_' +
                ...
                setID_file.to_csv(f, sep=',', index=False,
                                header=f.tell() == 0)
            writefile(protid, out_dir, pident, isoform, consequence, mapped_variants, 'MappedVariants', csv, hdf)
            # or 'batch_1.arrow'
